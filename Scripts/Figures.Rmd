...
Different way of visualizing the prevalence plots
```{r}
#load data for the prevalence plot from the cohort study group
load('M:/Plotting_Data/RC_prev_m1b.Rdata')

p3b<-ggplot()+
  geom_errorbar(data=rc_prev_df, aes(x=aid_status, y=ad_prev*100, 
                                       ymin=lower_CI*100, ymax=upper_CI*100, 
                                       color=gender, group=aid_status),
                  size=1, width=0.2, position=position_dodge2(0.2))+
  geom_point(data=rc_prev_df, aes(x=aid_status, y=ad_prev*100, color=gender, fill=gender), 
             size=4, shape = 21, position=position_dodge2(0.2))+
  scale_x_discrete(labels=c("Autoimmune", "Control"))+
  labs(color="Sex")+
  ylab("AD prevalence (%)")+
...
```{r}
#save the newer version of the plot
ggsave(filename="M:/Figures/RCprev.png", plot=p3b, device='png', dpi=600, 
       width=10, height=6.0)
ggsave(filename="~/EHR/toBox/RCprev.png", plot=p3b, device='png', dpi=600, 
       width=10, height=6.0)
```
...
Different way of visualizing the prevalence plot
```{r}
#load prevalence data
rc_prev_df<-select(read.csv('M:/Plotting_Data/RC_prev_m1b_stanford.csv'), -c(X))
...
```{r}
#save the newer version of the plot
ggsave(filename="M:/Figures/RCprev_stanford.png", plot=p12b, device='png', dpi=600, 
       width=10, height=6.0)
ggsave(filename="~/EHR/toBox/RCprev_stanford.png", plot=p12b, device='png', dpi=600, 
       width=10, height=6.0)
```
