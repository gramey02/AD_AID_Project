...
library(ggsci)
library(pwr)
###change this code here. Inputs are the matched_rc_clean dataframe and the keywords dataframe
#load cohort study data & disease keywords
load("M:/Matched_data/matched_rc_clean_2023_08_04_m1b.Rdata")
rc<-rc_matched
...
for(i in 1:length(group_names)){
  ...
  group_df<-rc %>% filter(subclass %in% df_filt$subclass) #also include the corresponding controls
...
start<-grep(aid_names[1], colnames(rc))[1] #get the column index where the first individual disease starts
for(i in 1:length(aid_names)){
  #filter the dataframe to only include the cur disease group
  print(i)
  print(aid_names[i])
  df_filt<-rc %>% filter(rc[,start+i-1]==1)
  aid_df<-rc %>% filter(subclass %in% df_filt$subclass)
  ...
  {#in the full group
    ...
    } else if(ncol(tbl)<2){ #skip if the table is not full
      print(paste("check ", aid_names[i], " table, idx: ", as.character(cur_idx), sep=""))
    } else {
      #fill in the dataframe
      df$a[cur_idx]<-tbl[2,2] #tbl[4]
  ...
  {#in males
    ...
    if(ncol(tbl)==1){
      df$d[cur_idx]<-tbl[2,1]
      df$b[cur_idx]<-tbl[1,1]
    } else if(nrow(tbl)<2){#skip if the table is empty
      print(paste("check ", aid_names[i], " table, idx: ", as.character(cur_idx), sep=""))
    } else if(ncol(tbl)<2){
      print(paste("check ", aid_names[i], " table, idx: ", as.character(cur_idx), sep=""))
    } else {
...
#the number of exposed and non-exposed cases is equal here, so use the pwr.2p test
...
for(i in 1:length(df$name)){
  ...
  if(is.na(df$h[i])==FALSE){
    if((df$a[i] + df$b[i])>=2 && (df$c[i] + df$d[i])>=2){
      cur_power<-(pwr.2p.test(h=ES.h(df$p1[i], df$p2[i]), n = df$a[i] + df$b[i], sig.level=0.05))[['power']]
...
write.csv(df, file="~/EHR/toBox/rc_power_stats.csv") 
